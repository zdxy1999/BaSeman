...
class MLC_Dataset_16c(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not self.testing: # 数据增强
            # TODO:我这边用npy的文件不能调数据增强的库
            #     需要调一个自己写的一个简单的数据增强函数，看另一个文件
            patch = utils.augmentation.data_augmentation(patch)
            #patch = patch*2-1#缩放到[-1,1]
            # print(hsi.shape)
...
class MLC_Dataset_16c_random_aug(torch.utils.data.Dataset):
