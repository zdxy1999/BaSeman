...
from .transformer_layers import SelfAttnLayer
from .utils import custom_replace,weights_init
from .backbone import Backbone_split_res18
...
class TwoBranchExtractor_res(nn.Module):
    def __init__(self,dims=[16,32,64,128,256,512],groups=16):
        super(TwoBranchExtractor_res,self).__init__()
        self.stem = nn.Conv2d(dims[0],dims[1],(4,4),(4,4),groups=groups)
        self.norm = nn.LayerNorm(dims[1])

        self.spectral_branch = nn.ModuleList()
        self.spatial_branch = Backbone_split_res18(16)
    ...
    def forward(self,x):
        temp = self.stem(x).permute(0,2,3,1)
        temp = self.norm(temp).permute(0,3,1,2)
        spectral_feature = temp
...
class SSNet(nn.Module):
    def __init__(self, in_channel=16,dims=[16,32,64,128,256,512], num_sem_tokens=64,hidden_size=512, heads=4,
        ...
        self.extactor = TwoBranchExtractor_res(dims,in_channel)
        self.semantic_tokenizer = SemanticTokenizer(num_sem_tokens,hidden_size,dims[-1])
        self.spectral_projector = nn.Conv2d(dims[-1],dims[-1]*in_channel,(4,4),(4,4),groups=in_channel)
