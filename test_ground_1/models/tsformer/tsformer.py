...
from models.tsformer.timm_models.vision_transformer import *
from models.tsformer.timm_models.util.layers import DropPath, Mlp
from models.tsformer.factory import register_model
...
class TSFormer(nn.Module):
    def __init__(self, pretrained, embed_dim=768, depth=12):
        super().__init__()
        self.img_encoder = vit_base_patch16_224_in21k(pretrained=pretrained, img_size=256)
        self.text_feats = torch.eye(17).cuda()
        self.blocks = nn.ModuleList([Block(dim=embed_dim, num_heads=1) for _ in range(depth)])
        self.cls_head = nn.Linear(embed_dim, 17)
        self.patch_head = nn.Linear(embed_dim, 17)
        self.text_head = TextHead(embed_dim, 17)
        self.sigmoid = nn.Sigmoid()
        #self.cfg = cfg
        self.text_linear = nn.Linear(17, embed_dim, bias=False)
        self.depth = depth
